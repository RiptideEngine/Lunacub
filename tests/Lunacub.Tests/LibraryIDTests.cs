...
public class LibraryIDTests {
    public static TheoryData<string> ValidParseIntegerString => [
        "0", "255", "65536", "2147483647", "18446744073709551615",
        " 255", "255 ", " 255 ",
    ];
    ...
    public static TheoryData<string> ValidParseHexadecimalString => [
        "0", "20", "FF", "FFFF", "FFFFFFFF", "FFFFFFFFFFFFFFFF",
    ...
    public static TheoryData<LibraryID, string> ToStringData => [
        (255, string.Empty),
        (255, "X"),
        (255, "X32"),
        (65535, string.Empty),
    ...
    public void Conversion_FromUInt128_ShouldBeCorrect() {
        LibraryID id = (ulong)25;
        id.Value.Should().Be(25);
    }

    [Fact]
    public void Converstion_ToUInt128_ShouldBeCorrect() {
        ((ulong)LibraryID.Parse("255")).Should().Be(255);
    }

    [Theory]
    [MemberData(nameof(ValidParseIntegerString))]
    public void ParseIntegerFromString_Valid_ShouldParsesCorrectly(string input) {
        new Func<LibraryID>(() => LibraryID.Parse(input)).Should().NotThrow().Which.Value.Should().Be(ulong.Parse(input));
        new Func<LibraryID>(() => LibraryID.Parse(input, null)).Should().NotThrow().Which.Value.Should().Be(ulong.Parse(input));
        new Func<LibraryID>(() => LibraryID.Parse(input.AsSpan(), null)).Should().NotThrow().Which.Value.Should().Be(ulong.Parse(input));
    }
    ...
    public void ParseIntegerFromString_Invalid_ShouldThrowsFormatException(string input) {
        new Func<LibraryID>(() => LibraryID.Parse(input)).Should().Throw<FormatException>();
        new Func<LibraryID>(() => LibraryID.Parse(input, null)).Should().Throw<FormatException>();
        new Func<LibraryID>(() => LibraryID.Parse(input.AsSpan(), null)).Should().Throw<FormatException>();
    }
    ...
    public void ParseIntegerFromString_Negative_ShouldThrowsOverflowException(string input) {
        new Func<LibraryID>(() => LibraryID.Parse(input)).Should().Throw<OverflowException>();
        new Func<LibraryID>(() => LibraryID.Parse(input, null)).Should().Throw<OverflowException>();
        new Func<LibraryID>(() => LibraryID.Parse(input.AsSpan(), null)).Should().Throw<OverflowException>();
    }
    ...
    public void ParseHexadecimalFromString_Valid_ShouldParsesCorrectly(string input) {
        new Func<LibraryID>(() => LibraryID.Parse(input, NumberStyles.HexNumber)).Should().NotThrow().Which.Value.Should().Be(ulong.Parse(input, NumberStyles.HexNumber));
    }
    ...
    public void ParseHexadecimalFromString_Invalid_ShouldThrowsFormatException(string input) {
        new Func<LibraryID>(() => LibraryID.Parse(input, NumberStyles.HexNumber)).Should().Throw<FormatException>();
    }
    ...
    public void TryParseIntegerFromString_Valid_ShouldReturnsTrueAndCorrectValue(string input) {
        new Func<(bool, LibraryID)>(() => {
            bool parse = LibraryID.TryParse(input, NumberStyles.Integer, null, out LibraryID output);
            return (parse, output.Value);
        }).Should().NotThrow().Which.Should().Be((true, ulong.Parse(input)));
        new Func<(bool, LibraryID)>(() => {
            bool parse = LibraryID.TryParse(input, null, out LibraryID output);
            return (parse, output);
        }).Should().NotThrow().Which.Should().Be((true, ulong.Parse(input)));
    }
    ...
    public void TryParseIntegerFromUtf8_Valid_ShouldReturnsTrueAndCorrectValue(byte[] input) {
        new Func<(bool, LibraryID)>(() => {
            bool parse = LibraryID.TryParse(input, null, out LibraryID result);
            return (parse, result);
        }).Should().NotThrow().Which.Should().Be((true, ulong.Parse(input)));
        
        new Func<(bool, LibraryID)>(() => {
            bool parse = LibraryID.TryParse(input, NumberStyles.Integer, null, out LibraryID result);
            return (parse, result);
        }).Should().NotThrow().Which.Should().Be((true, ulong.Parse(input)));
    }
    ...
    public void TryParseHexadecimalFromString_Valid_ShouldReturnsTrueAndCorrectValue(string input) {
        new Func<(bool, LibraryID)>(() => {
            bool parse = LibraryID.TryParse(input, NumberStyles.HexNumber, null, out LibraryID result);
            return (parse, result);
        }).Should().NotThrow().Which.Should().Be((true, ulong.Parse(input, NumberStyles.HexNumber)));
    }
    ...
    public void ToString_ShouldReturnsCorrectly(LibraryID id, string format) {
        id.ToString(format).Should().Be(id.Value.ToString(format));
    }
    
    [Fact]
    public void Equality_ShouldReturnsCorrectly() {
        ((LibraryID)255).Should().Be((LibraryID)255);
        ((LibraryID)255 == (LibraryID)255).Should().BeTrue();
        ((LibraryID)255 != (LibraryID)255).Should().BeFalse();
        
        ((LibraryID)255).Should().Be((ulong)255);
        ((LibraryID)255 == (ulong)255).Should().BeTrue();
        ((LibraryID)255 != (ulong)255).Should().BeFalse();
    }
    
    [Fact]
    public void Inequality_ShouldReturnsCorrectly() {
        LibraryID.Parse("255").Should().NotBe((ulong)256U);
        
        ((LibraryID)255 != (LibraryID)257).Should().BeTrue();
        ((LibraryID)255 != (ulong)255).Should().BeFalse();
    }
}